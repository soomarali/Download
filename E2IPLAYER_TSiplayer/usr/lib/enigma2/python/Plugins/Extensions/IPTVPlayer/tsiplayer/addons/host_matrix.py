...
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.matrix.resources.lib.tmdb import cTMDb
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.matrix.resources.lib.home import cHome
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.matrix.resources.lib.util import UnquotePlus
MAIN_URL0   = '/usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/tsiplayer/addons/matrix/resources/sites'
...
def _pluginSearch(plugin, sSearchText):
    try:  
        plugins = __import__('Plugins.Extensions.IPTVPlayer.tsiplayer.addons.matrix.resources.sites.%s' % plugin['identifier'], fromlist=[plugin['identifier']])
...
class TSIPHost(TSCBaseHostClass):
    def __init__(self):
        ...
        self.MAIN_URL      = '/usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/tsiplayer/addons/matrix'
        ...
        for (dir_) in lst:
            if (dir_.endswith('.py')) and ('init' not in dir_) and ('globalSources' not in dir_)and ('globalSearch' not in dir_):
                if os.path.exists('/usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/tsiplayer_local/'):
                    ...
                    elm = ['plugin://plugin.video.matrix/', '13', 'site='+file_+'&siteUrl=&sTitleWatched=']
                    sys.argv = elm                
                    image   = 'file:///usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/tsiplayer/addons/matrix/resources/art/sites/'+file_+'.png'
    ...
    def showmenu1(self,cItem):
        ...
        sys.argv    = cItem.get('argv',['plugin://plugin.video.matrix/', '13', '?'])   
    ...
    def showResult(self,cItem):
        ...
        for Items in listing:           
            ...
            icon           = icon.replace('plugin.video.xxxx','plugin.video.matrix')
            icon           = icon.replace('special://home/addons/plugin.video.matrix/resources/','file:///usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/tsiplayer/addons/matrix/resources/')
            ...
            argv           = ['plugin://plugin.video.matrix/', '13','?'+sParams]            
            Year           = ''
            ...
            if ('Tools' != sTitle.strip()) and ('My accounts' != sTitle.strip()) and ('Continue watching' != sTitle.strip()) and ('Search history' != sTitle.strip()) and ('Bookmarks' != sTitle.strip()):
